...
WORKDIR /src
COPY ["TestNSwagFunction.csproj", "."]
RUN dotnet restore "./TestNSwagFunction.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "TestNSwagFunction.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "TestNSwagFunction.csproj" -c Release -o /app/publish /p:UseAppHost=false
